      ...
      <div>
        <nav className={`${inter.className} hidden md:flex flex-wrap items-center justify-center gap-4 md:gap-6`}>
          <div className="relative">
            <button type="button" onClick={() => setIsProdutoOpen(!isProdutoOpen)} className="text-sm font-medium hover:opacity-70 transition">Produtos</button>
            {isProdutoOpen && (
              <div className="absolute top-full left-0 z-50 mt-4 w-[350px] rounded-[28px] border border-white/10 bg-zinc-950 p-4 shadow-2xl">
                <div className="grid grid-cols-1 items-stretch gap-4">
                  <div className="grid h-full grid-rows-2 gap-3">
                    ...
                    <a href="#gestãodetráfego" className="block h-full rounded-2xl border border-white/15 bg-[#14141a] px-4 py-3 transition hover:bg-[#1b1b22]">
                      <p className="text-base font-medium text-white">Gestão de tráfego</p>
                      <p className="mt-1 text-sm leadind-6 text-white/60">Anúncios estratégicos para aumentar sua visibilidade e trazer mais clientes para o seu negócio.</p>
                    </a>
                  </div>
                </div>
              </div>
      ...
      <div className={`${inter.className} md:hidden mt-5 flex flex-col gap-3 rounded-[28px] border-t border-white/10 bg-black/80 px-4 pt-4 pb-5 shadow-2xl backdrop-blur-md`}>
        ...
        {isMobileProdutoOpen && (
          <div className="mt-3 overflow-hidden rounded-[24px] border border-white/10 bg-zinc-950">
            ...
            <a href="#gestãodetráfego" onClick={() => { setIsMenuOpen(false); setIsMobileProdutoOpen(false); }} className="block px-4 py-4 transition hover:bg-white/5">
              <p className="text-base font-medium text-white">Gestão de tráfego</p>
              <p className="mt-1 text-sm leading-6 text-white/60">Anúncios estratégicos para aumentar sua visibilidade e trazer mais clientes para o seu negócio.</p>
            </a>
          </div>
        )}
